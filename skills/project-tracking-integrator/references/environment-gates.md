...
当三个环境写入同一个分析项目时，每一份浏览器证据必须携带环境名；每一份入库证据必须携带实际执行的环境名、过滤属性和过滤值。最终报告会把入库证据与契约 profile 逐项核对，缺失或不一致时按 `QUERY_FAILED` 处理。结果键使用：
...
每个环境都使用环境值、事件名和覆盖浏览器触发时刻的短时间窗查询候选数据，在本地应用稳定 match 关联业务动作，再严格比较契约声明的字段和次数。平台返回但契约未声明的字段不参与门禁结论。SDK 全局对象不可见本身也不构成门禁失败。
...
生产环境只执行无支付、无删除、无发布、无外部消息、无不可逆数据变化的最小旅程。任何浏览器操作之前必须运行旅程校验器并读取其引用的契约；事件未显式声明 `smokeSafe: true`、生产 origin 不一致或契约不可读时，立即返回 `BLOCKED`，不能打开或操作生产旅程。
