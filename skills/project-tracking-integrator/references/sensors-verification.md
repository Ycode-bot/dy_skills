...
多个业务动作共用 `ima_function_click` 等事件名时，必须用 `targets.sensors.match` 指定稳定的业务区分字段，否则同一时间窗内的其他点击会被误判为重复上报。验证器允许字符串、数字或布尔值的文本等价形式先完成候选关联，例如文档的字符串 `"2"` 可以关联平台返回的数字 `2`；这不代表验收通过，最终类型和值仍由 `properties` 严格判断。任何需要验证类型或取值的 match 字段都必须同时写入 `properties`。`match` 只接受字符串、有限数字或布尔值。
