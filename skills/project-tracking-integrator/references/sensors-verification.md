...
环境值不能代替测试身份。当环境或事件要求身份时，优先使用 `distinct_id + 事件名 + 稳定 match 字段 + 短时间窗`。若浏览器自动化运行在隔离上下文、无法安全读取 SDK identity，可使用 `--discover-identity` 做受限发现；只有唯一身份才能继续比较，多个或缺失身份返回 `BLOCKED`。
...
浏览器能够完成业务动作、但页面全局 SDK handle 在自动化上下文中不可见时，不要判定 `NOT_SENT`，也不要跳过查询。使用受限身份发现：
...
该模式要求环境过滤和每个事件至少一个稳定 match 字段，自动把每个契约查询的窗口限制为最多 10 分钟、结果限制为最多 20 条。零条候选返回 `NOT_FOUND`；唯一 `distinct_id` 会被用于契约与次数比较但不会写入报告；多个身份或结果缺少身份返回 `BLOCKED`。
...
| `BLOCKED` | 受限身份发现命中多个身份，或候选事件缺少 `distinct_id`，无法唯一归属本次操作 |
...
- 只使用只读、最小权限凭证。
- 优先验证测试环境和测试账号。
- 优先使用 `distinct_id + 短时间窗 + event` 缩小查询范围；身份不可观测时只允许环境、稳定 match、每个契约最多 10 分钟和 20 条结果的受限发现。
