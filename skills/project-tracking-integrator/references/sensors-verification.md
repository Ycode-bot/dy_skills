...
把事件名、环境、时间窗和稳定 match 字段只作为查询定位条件。查询成功后，只比较数据需求声明的属性和次数；神策返回但需求未声明的字段不参与结论。需求明确列出的任何字段都通过同一套普通属性规则验证，不设置平台专用字段门槛。
...
默认使用环境值、事件名、稳定 match 字段和短时间窗定位待验收数据，再按需求契约比较字段与次数。
...
两种模式都生成短日期范围、单事件、稳定业务条件和小结果集 SQL。
...
浏览器能够完成业务动作、但页面全局 SDK handle 在自动化上下文中不可见时，不要判定 `NOT_SENT`，也不要跳过查询。直接按环境、事件、稳定 match 和覆盖该动作的时间窗执行同一查询。
...
- 只使用只读、最小权限凭证。
- 优先验证测试环境和测试账号。
- 使用环境、事件名、稳定 match 和短时间窗缩小查询范围，再只比较需求契约声明的字段与次数。
