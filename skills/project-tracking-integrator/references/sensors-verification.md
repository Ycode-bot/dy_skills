...
默认使用环境值、事件名、稳定 match 字段和短时间窗完成验收。神策会为游客和登录用户的事件写入 `distinct_id`，但验证器不要求调用方提前获取或匹配它。只有用户明确要求核对某个已知身份时，才额外使用 `--distinct-id`。
...
| `targets.sensors.distinctId` | 可选的已知身份过滤；默认验收不需要 |
...
浏览器能够完成业务动作、但页面全局 SDK handle 在自动化上下文中不可见时，不要判定 `NOT_SENT`，也不要跳过查询。直接按环境、事件、稳定 match 和覆盖该动作的时间窗执行同一查询。`--distinct-id` 仅是用户提供已知身份时的可选缩小条件。
...
| `NOT_FOUND` | 未找到符合事件、环境、稳定字段和时间窗的入库数据 |
...
- 只使用只读、最小权限凭证。
- 优先验证测试环境和测试账号。
- 使用环境、事件名、稳定 match 和短时间窗缩小查询范围；只有用户明确要求已知身份时才增加 `distinct_id` 过滤。
...
数据接收成功不等于立即可查询。先确认项目、环境、事件 match 和时间窗，再确认是否存在入库延迟。不要无限重试或扩大到全量查询。
...
同名事件可能来自旧版本、其他入口或重复触发。验收必须同时核对环境、稳定 match、时间窗、属性契约和数量约束。
