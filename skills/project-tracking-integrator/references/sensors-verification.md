...
神策 SQL 只用事件名、环境和短时间窗获取候选数据。查询成功后，在本地用稳定 `match` 字段关联同名业务动作，再严格比较数据需求声明的属性和次数；神策返回但需求未声明的字段不参与结论。需求明确列出的任何字段都通过同一套普通属性规则验证，不设置平台专用字段门槛。
...
只有在 `full-lifecycle` 模式中，四者一致时才判定完整交付完成。代码存在不代表调用路径可达；浏览器请求成功也不代表事件已经入库；神策存在同名事件也不代表字段、类型、枚举和触发次数正确。
...
默认使用环境值、事件名和短时间窗从神策获取待验收候选数据，再在本地应用稳定 `match` 并按需求契约严格比较字段与次数。
...
| `targets.sensors.match` | 在本地从同名候选事件中关联业务动作的稳定字段，如 `btn_name` |
...
- 字段名、必填性和“资料类型”进入 `properties`。
- 示例值不得自动进入 `match`、`equals`、`oneOf`、`pattern`、浏览器断言或实现常量。
- 只有标为“固定值”“必须等于”“枚举”“映射规则”，或用户明确指定为本次测试期望值的内容，才成为值约束。
- 例如 `page_content_no` 的类型是 `string`，示例写 `2`，实际入库为字符串 `3` 时，类型验收通过；不能仅因示例不同判为异常。
- 产品只需提供业务动作和数据文档，不需要解释代码内部如何计算位置编号。若精确编号是验收要求，应由数据文档另列绑定规则，而不是依赖示例列。

多个业务动作共用 `ima_function_click` 等事件名时，优先用数据文档明确的固定业务标识、本次测试用例值或与被触发 UI 元素直接关联的稳定标识作为 `targets.sensors.match`，否则同一时间窗内的其他点击可能被误判为重复上报。不要把示例列中的位置编号用作 `match`。验证器允许字符串、数字或布尔值的文本等价形式先完成候选关联；这不代表验收通过，最终类型和绑定值仍由 `properties` 判断。任何需要验证类型或取值的 match 字段都必须同时写入 `properties`。`match` 只接受字符串、有限数字或布尔值。
...
两种模式都生成短日期范围、单事件、环境条件和小结果集 SQL。稳定业务 `match` 不拼进 SQL，而是在本地候选行中应用，避免神策列类型与数据文档不一致时直接触发查询引擎错误。
...
- 只使用只读、最小权限凭证。
- 优先验证测试环境和测试账号。
- 使用环境、事件名和短时间窗缩小平台候选查询范围，在本地应用稳定 match，再只比较需求契约声明的字段与次数。
...
数据接收成功不等于立即可查询。先确认项目、环境、事件、时间窗和本地 match，再确认是否存在入库延迟。不要无限重试或扩大到全量查询。
...
同名事件可能来自旧版本、其他入口或重复触发。平台先按环境、事件和时间窗返回候选；验收再在本地核对稳定 match、属性契约和数量约束。若平台实际类型与文档不同，应报告 `CONTRACT_MISMATCH`，不能通过修改文档类型或把业务字段塞进 SQL 来绕过。
