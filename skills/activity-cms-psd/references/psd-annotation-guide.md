# Activity CMS PSD 标注规范

本文档给 UI 和运营使用。第一阶段目标是让 Skill 稳定完成复杂 PSD 的切图、主题色提取、真实 CMS 组件识别，并生成可导入 activityincms 的整页 JSON。

## 核心原则

切图就是切图，组件就是组件。

不要把“要导出的图片”和“后台组件类型”写进同一个复杂名字里。Skill 会分别识别：

- `切图:`：一定要导出的图片素材。
- `组件:`：这个区域要生成哪个真实 CMS 组件。

二者不强制写在同一个名字里，但要一起交付：`组件:` 决定 JSON 里的真实组件，`切图:` 决定运营后续替换组件图片字段时可用的素材包。

## 切图命名

格式：

```txt
切图:<素材名>
切图:<素材名>[宽x高]
切图:<素材名>(后台字段说明/尺寸/留白/显示要求)
```

示例：

```txt
切图:头图
切图:头图[750x300]
切图:规则背景
切图:抽奖标题[700x180]
切图:奖池进度
切图:规则弹窗-图片(宽度688,上方留出部分空白)
切图:弹窗-标题背景图(692*166)
切图:弹窗-内容装饰(739*496,清空不显示)
```

规则：

- 只有 `切图:` 开头的图层或组会被强制导出 PNG。
- `[宽x高]` 是可选的。
- `(...)` 或 `（...）` 可以写运营/CMS 后台字段说明、尺寸、留白和显示要求。
- 导出的文件名只使用括号前的素材名；括号内容会进入 JSON `meta`、debug 报告和交接 todo，不进入文件名。
- 不写尺寸时，按 PSD 图层/组的实际 bounds 导出。
- 写完整尺寸时，Skill 会按目标宽高导出，例如 `739*496` 或 `宽度739 高度496`。
- 只写单个尺寸时，只调整该维度，另一个维度保留 PSD 图层/组原始尺寸，例如 `宽度688` 会导出为 `688 x 原始高度`。
- `上方留出部分空白`、`清空不显示` 等非尺寸要求只作为交接提示记录，不会自动裁切、补透明区、隐藏内容或修改像素。
- 尺寸格式统一使用半角或全角括号都可以：`[750x300]`、`［750x300］`。
- 尺寸分隔符支持 `x`、`X`、`×`、`*`。

## 组件命名

格式：

```txt
组件:<中文组件名>
```

示例：

```txt
组件:奖池升级
组件:倒计时
组件:榜单
组件:任务
组件:报名
组件:礼物兑换
```

规则：

- `组件:` 会进入 `cms-page-config.json`，生成真实 activityincms 组件。
- 组件识别结果会写入 `cms-page-config.json`；如果研发使用 `--debug`，也会写入 `inspect/component-detection.json` 方便追溯。
- 如果组件依赖后台 ID 或接口数据，Skill 会把相关字段留空，并在 `meta.todos` 中提示运营补齐。
- 业务 ID 不写在 PSD 里，由运营后续在 CMS 或 JSON 中补。

常用映射：

| PSD 标注 | CMS componentName | 说明 |
|---|---|---|
| `组件:奖池升级` | `drawPool2` | 274 - 奖池升级 节日主题 |
| `组件:倒计时` | `countDown` | 倒计时组件 |
| `组件:榜单` | `commonGiftRank` | 通用礼物榜单候选 |
| `组件:任务` | `taskDraw` | 任务抽奖候选 |
| `组件:报名` | `signUp2` | 报名组件候选 |
| `组件:礼物兑换` | `giftExchange` | 礼物兑换候选 |
| `组件:盲盒` | `blackbox` | 盲盒抽奖 |

## 推荐 PSD 结构

```txt
画板 1
  切图:头图[750x900]
  切图:倒计时背景
  组件:倒计时

  切图:主选项卡
  组件:奖池升级
  切图:飘屏中奖通告
  切图:抽奖标题[700x180]
  切图:抽奖面板
  切图:奖池标题[750x180]
  切图:奖池进度

  组件:榜单
  切图:榜单背景

  切图:规则背景
```

## 业务 ID

UI 不需要提供后台 ID。

不要在 PSD 中写：

```txt
actId
actIdTest
testId
rankId
rankTestId
taskId
signupId
drawId
```

这些由运营在 CMS 或 JSON 中补齐。

## 错误示例

### 一个大组切全页

```txt
切图:整页
  所有内容
```

问题：运营后续无法替换局部素材，也无法判断组件结构。

### 尺寸写在备注里

```txt
头图 750 300
```

问题：Skill 无法稳定解析。正确写法：

```txt
切图:头图[750x300]
```

### 把组件当切图

```txt
切图:奖池升级
```

如果只是要素材，可以这样写；如果要让 JSON 里生成真实后台组件，还需要单独写：

```txt
组件:奖池升级
```

## 兼容旧格式

旧格式仍可识别，但不再推荐给 UI 使用：

```txt
cms:drawPool2#upgradePrizePool
asset:drawImg
text:drawText1
style:contBg
```

后续新 PSD 优先使用中文前缀。
